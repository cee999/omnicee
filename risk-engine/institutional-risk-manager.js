...
const MAX_CORRELATION_EXPOSURE = 0.15;
const KELLY_FRACTION = 0.25;
const MIN_LIQUIDITY_RATIO = 0.001;
const TAIL_RISK_CONFIDENCE = 0.95;
...
class KellyCriterionCalculator {
  static calculate(winRate, avgWin, avgLoss, accountBalance, currentPrice) {
    ...
    const cappedKelly = clamp(quarterKelly, 0, 0.10);
...
class CorrelationAnalyzer {
  constructor() {
    this.returnsHistory = new Map();
    this.correlationMatrix = new Map();
  }

  addReturns(symbol, returns) {
    this.returnsHistory.set(symbol, returns.slice(-100));
...
class InstitutionalRiskManager extends EventEmitter {
  constructor(config = {}) {
    ...
    // FIX: was never fed real trade results anywhere, so KellyCriterionCalculator .adaptiveKelly() always fell back to its cold-start default (tradeHistory.length < 10) and CorrelationAnalyzer never had...
  ...
  // FIX: this whole class was instantiated + connected in index.js but .validateAndSizePosition() was never called anywhere, and nothing ever fed it real trade outcomes — see index.js and api/server.js...
  ...
  validateAndSizePosition(signal, currentPrice) {
    ...
    // FIX: MAX_PORTFOLIO_RISK and MAX_CORRELATION_EXPOSURE were defined at the top of this file but never referenced anywhere — the intended portfolio-wide exposure cap and correlated-exposure cap were...
