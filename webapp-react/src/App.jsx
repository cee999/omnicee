...
function ValidTab({ signals, journalStats, learningProfiles, mode }) {
  ...
  const avgMc = avg(mcWinProbs), avgBayes = avg(bayesPosteriors), avgWfe = avg(wfeVals);
  ...
  const grades = journalStats?.byGrade ? Object.entries(journalStats.byGrade) : [];
  const topLearning = learningProfiles ? [...learningProfiles].sort((a, b) => b.expectancyR - a.expectancyR).slice(0, 8) : null;

  return (
    <div className="p-4 space-y-4">
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <div className="omni-panel p-3 flex flex-col items-center justify-center">
          {avgWfe != null ? <Gauge value={avgWfe * 100} label="Avg Walk-Forward Eff." /> : <span className="font-mono text-[10px] text-center" style={{ color: 'var(--textFaint)' }}>Walk-forward: no data yet</span>}
        </div>
        <div className="omni-panel p-3 flex flex-col items-center justify-center">
          {avgBayes != null ? <Gauge value={avgBayes * 100} label="Avg Bayesian Posterior" /> : <span className="font-mono text-[10px] text-center" style={{ color: 'var(--textFaint)' }}>Bayesian: no data yet</span>}
        </div>
        <div className="omni-panel p-3 flex flex-col items-center justify-center">
          {avgMc != null ? <Gauge value={avgMc} label="Avg Monte Carlo Win %" /> : <span className="font-mono text-[10px] text-center" style={{ color: 'var(--textFaint)' }}>Monte Carlo: no data yet</span>}
        </div>
        <div className="omni-panel p-3 flex flex-col items-center justify-center">
          {kellyPct != null ? (
            <>
      ...
      <div className="omni-panel p-4">
        ...
        {hasJournal ? (
          <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
            ...
            <StatCard label="Profit Factor" value={journalStats.pf === 999 ? '∞' : journalStats.pf} accent="var(--gold)" />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className="omni-panel p-4">
          <SectionHeader icon={CheckCircle2} title="Performance by Grade" sub={hasJournal ? `${journalStats.total.toLocaleString()} trades` : undefined} />
          {grades.length ? (
            <div className="space-y-2.5">
              {grades.map(([grade, g]) => (
    ...
    <div className="omni-root omni-boot flex flex-col h-full min-h-[640px] w-full text-sm">
      ...
      <div className="flex flex-col flex-1 min-h-0">
        <div className="flex-1 overflow-y-auto omni-scroll">
          {activeTab === 'DASH' && <DashTab signals={feed.signals} equityCurve={feed.equityCurve} prices={feed.prices} changes={feed.changes} accountBalance={feed.accountBalance} journal={feed.journalStats} />}
          ...
          {activeTab === 'RISK' && <RiskTab prices={feed.prices} changes={feed.changes} accountBalance={feed.accountBalance} relativeStrength={feed.relativeStrength} mode={feed.mode} />}
        </div>
